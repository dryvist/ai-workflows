...
Not advisory. If the router is unreachable the job probes it three times
inside fifteen seconds and then fails, releasing the runner; the job itself
is capped at ten minutes (`timeout-minutes`). A wrong key, base URL or model alias fails in seconds instead of waiting.
